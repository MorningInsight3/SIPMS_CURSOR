...
export default function ActiveMonitoring() {
  ...
  const gotoFindings = () => {
    router.push({
      pathname: '/findings',
      params: { type: 'AM' }, // Passing the dynamic repoId
    });  };
  ...
  return ( 
    <SafeAreaView>
      <ScrollView style={tw`mx-4 my-2 bg-white rounded-5 h-full p-5`}>
        <Header title={"Active Monitoring"} />

        <Ripple onPress={gotoFindings}
            style={tw` px-[10px] rounded-[10px] bg-[#000] text-white`}
          >
            <Button textColor="white" style={tw`py-0`}>
              <CustomText style={tw`font-semibold text-[12px]`}>View Active Monitoring Data</CustomText>
            </Button>
          </Ripple>
            ...
            data.all_items.map((item) => (
              <Link
                ...
                href={{
                  ...
                  params: { repoId: item.id, Type: 'activemonitoring' }, // Passing the dynamic repoId
                }} 
              >
